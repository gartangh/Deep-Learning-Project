...
class RandomPolicy:
    ...
    def get_action(self, board, legal_actions):
        return random.choice(list(legal_actions.keys()))

    def optimal(self, board, legal_actions):
...
class EpsGreedyPolicy:
    ...
    def get_action(self, board, action_value_network, legal_actions):
    ...
    def optimal(self, q_values, legal_actions):
...
class AnnealingEpsGreedyPolicy:
    ...
    def get_action(self, board, action_value_network, possible_actions):
        self.decisions_made += 1
        return self.get_current_eps_policy().get_action(board, action_value_network, possible_actions)

    #TODO: remove this method and change with 2 policies in your DQN algo: one target_policy and one optimizing_policy
    def optimal(self, q_values, possible_actions):
        return self.get_current_eps_policy().optimal(q_values, possible_actions)
